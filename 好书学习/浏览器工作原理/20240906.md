...
## 执行顺序
- 执行主线程中的所有同步任务，这些任务会依次按顺序完成。
- 执行所有的微任务（Microtasks，或叫作微任务队列中的任务），这些任务会在当前宏任务（Macrotask）执行完后立即执行。
- 进行页面的重新渲染（如果在浏览器环境中运行）。
- 执行一个宏任务（从宏任务队列中取出一个），例如 setTimeout 或 setInterval 的回调。
- 重复以上步骤。
